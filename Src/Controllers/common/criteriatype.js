...
exports.findAll = async (req, res) => {
  ...
  let condition = criteriaType
    ? {
        criteriaType: { $regex: new RegExp(criteriaType), $options: "i" },
        status: "Active",
      }
    : { status: "Active" };

  await CriteriaType.find(condition)
    .sort({ criteriaType: 1 })
    .then((data) => {
      res.send({
        status: "200",
...
exports.update = async (req, res) => {
  ...
  await CriteriaType.findByIdAndUpdate(id, req.body,{status: "Active" }, {
...
exports.findOne = async(req, res) => {
  ...
  await CriteriaType.findOne({ _id: id, status: "Active" })

      .then(data => {
          if (!data)
              res.status(404).send({ message: "Not found CriteriaType with id " + id });
          else res.send(data);
      })
      .catch(err => {
          res
              .status(500)
              .send({ message: "Error retrieving CriteriaType with id=" + id ||err.message});
      });
};
